...
#functions
source("~/Library/CloudStorage/Box-Box/BBL/ms_generic_scripts/msdepression_functions.R")
```
...
6. Supplement: 
 - Promis tables
#Rstduio Version: 2024.04.1+748 (2024.04.1+748)
...
###### Lesion & brainvol by depGroupVar (lesion vol - Y, tbv - N, N.S)
```{r UF_by_depGroupVar}
...
table(df_dep$depDiagnosis) #MS+Dep n = 207; MS+noDep n = 99
 ...
 #mean vol mimosa lesions, depDiagnosisMS+noDepression 
#(T = -3.164, P = 0.002, Cohen's f2 =  0.128, Effect size = small)
dep_mean_lesion_vol_gam_prop <- gam(volume_of_mimosa_lesions ~ depDiagnosis + osex + s(PAT_AGE_AT_EXAM, k=4, fx=F), data = df_dep)
summary(dep_mean_lesion_vol_gam_prop)

  #print summary stats
 gam_summary_stats(dep_mean_lesion_vol_gam_prop)
 
  #mean brain vol mimosa lesions, depDiagnosisMS+noDepression  
 #(T = 0.016, P = 0.988, Cohen's f2 =  0.333, Effect size = medium)
dep_mean_total_brain_vol_gam_prop <- gam(total_volume ~ depDiagnosis + osex + s(PAT_AGE_AT_EXAM, k=4, fx=F), data = df_dep)
summary(dep_mean_total_brain_vol_gam_prop)

  #print summary stats
 gam_summary_stats(dep_mean_total_brain_vol_gam_prop)
...
#First, test if they are related. If so, they can't really be modeled together -> 
  ...
  corr_LV_to_UF <- cor.test(uncinate_by_dx_df$volume_of_mimosa_lesions,uncinate_by_dx_df$mean_UF_vol) #corr 0.69; 
  print(corr_LV_to_UF)
  
  lesion_vol_UF_lm <- lm(uncinate_by_dx_df$volume_of_mimosa_lesions~ uncinate_by_dx_df$mean_UF_vol) #P <2.2*10^-16, they are colinear
  summary(lesion_vol_UF_lm) #T=10.585
  
  #how about in fornix
  
  corr_LV_to_fornix <- cor.test(uncinate_by_dx_df$volume_of_mimosa_lesions,uncinate_by_dx_df$mean_fornix_volume) #corr 0.69; 
  print(corr_LV_to_fornix) #0.3452654 
  
  lesion_vol_fornix_lm <- lm(uncinate_by_dx_df$volume_of_mimosa_lesions~ uncinate_by_dx_df$mean_fornix_volume)
  summary(lesion_vol_fornix_lm) #T = 4.047

  ## Try scaling mean volume
  disease_mean_scaled_UF_vol_gam <- gam(mean_UF_vol_scaled_total_vol_lesions ~ Diagnosis + osex+s(PAT_AGE_AT_EXAM, k = 4, fx = F), data = uncinate_by_dx_df)
  summary(disease_mean_scaled_UF_vol_gam) #- N.S.
  
  #print summary stats; (T = -0.556, P = 0.579, Cohen's f2 =  0.007, Effect size = minimal)"
  gam_summary_stats(disease_mean_scaled_UF_vol_gam)
  
  #How about predicting Diagnosis? - nope
  diagnosis_prediction <- glm(Diagnosis ~ mean_UF_prop + Diagnosis + osex+PAT_AGE_AT_EXAM + volume_of_mimosa_lesions, data = uncinate_by_dx_df, family = "binomial")
  summary(diagnosis_prediction)
  confint(diagnosis_prediction)

  #how about predicting diagnosis with scaled UF? N.S.
  diagnosis_prediction_scaled <- glm(Diagnosis ~ mean_UF_vol_scaled_total_vol_lesions + Diagnosis + osex+PAT_AGE_AT_EXAM, data = uncinate_by_dx_df, family = "binomial")
  summary(diagnosis_prediction_scaled)
  confint(diagnosis_prediction_scaled)
  
  
  
## modeling this doesn't make sense
#disease_mean_UF_prop_w_LV_gam <- gam(mean_UF_prop ~ Diagnosis + osex+s(PAT_AGE_AT_EXAM, k = 4, fx = F) + volume_of_mimosa_lesions, data = uncinate_by_dx_df)
#summary(disease_mean_UF_prop_w_LV_gam)
 
 #print summary stats
 # gam_summary_stats(disease_mean_UF_prop_w_LV_gam)
```
###### JAMA ANALYSIS-  UF Anxiety dose + lesion vol
```{r uncinateByAnxietyDose}
...
df_demo_and_fascicles_no_unclass_anxiety_dose <- df_demo_and_fascicles %>%
  ...
  filter(anxiety_dose != -1) #n=372
    ...
    #bilat anxiety dose + sex + age: anxiety_dose    0.04989    0.02397   2.082   0.0381 * original result

  #How about predicting Diagnosis? volume_of_mimosa_lesions  1.518e-05  7.147e-06   2.124   0.0357 *
  diagnosis_prediction <- lm(anxiety_dose ~ mean_UF_prop + osex+PAT_AGE_AT_EXAM + volume_of_mimosa_lesions, data = uncinate_by_dx_df)
  summary(diagnosis_prediction)
  confint(diagnosis_prediction)


### modeling volume of mimosa lesions like this doesn't make sense
 #anxietyGrouping_mean_UF_by_sex_w_LV_gam_prop <- gam(mean_UF_prop ~ anxiety_dose + osex + s(PAT_AGE_AT_EXAM, k = 4, fx = F) + volume_of_mimosa_lesions, data=df_demo_and_fascicles_no_unclass_anxiety_dose)
 # summary(anxietyGrouping_mean_UF_by_sex_w_LV_gam_prop)
 
  #print summary stats
  #gam_summary_stats(anxietyGrouping_mean_UF_by_sex_w_LV_gam_prop)
...
#First, test if they are related. If so, they can't really be modeled together -> 
 ...
 # t = -0.34681, df = 121, p-value = 0.7293; corr -0.03151255 
  ...
  total_vol_UF_lm <- lm(uncinate_by_dx_df$total_volume~ uncinate_by_dx_df$mean_UF_vol) #P <2.2*10^-16, they are colinear
  summary(total_vol_UF_lm) 
...
# cor: -0.06590961 ; t = -1.2706, df = 370, p-value = 0.2047
  ...
  total_vol_UF_lm <- lm(df_demo_and_fascicles_no_unclass_anxiety_dose$total_volume~ df_demo_and_fascicles_no_unclass_anxiety_dose$mean_UF_vol) #P <2.2*10^-16, they are colinear
  summary(total_vol_UF_lm) 
...
##### JAMA ANALYSIS - compare correlations w/wout uncinate
```{r other_injured_fascicles}
df_demo_and_fascicles_no_unclass_anxiety_dose <- df_demo_and_fascicles %>%
  filter(oanxietydx_OR_meds_AND_Anxietydx.Meds.And.Dep.True.Healthy.Collapsed != "unclassified") %>%
  filter(anxiety_dose != -1)  %>% #n=372 
  mutate(total_lesion_burden_no_UF = sum_fascicle_vol_lost - (UF_L_vol + UF_R_vol))

#almost identical because uncinate is small
#t=2.05, p = 0.04083
cor_lesion_burden_by_anxiety = cor.test(df_demo_and_fascicles_no_unclass_anxiety_dose$anxiety_dose, df_demo_and_fascicles_no_unclass_anxiety_dose$sum_fascicle_vol_lost)
print(cor_lesion_burden_by_anxiety)

#t=2.05, p = 0.04075
cor_lesion_burden_no_UF_by_anxiety = cor.test(df_demo_and_fascicles_no_unclass_anxiety_dose$anxiety_dose, df_demo_and_fascicles_no_unclass_anxiety_dose$total_lesion_burden_no_UF)
print(cor_lesion_burden_no_UF_by_anxiety)
