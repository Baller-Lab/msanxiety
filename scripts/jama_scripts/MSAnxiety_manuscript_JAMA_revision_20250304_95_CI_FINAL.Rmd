...
library(boot)
#functions
source("~/Library/CloudStorage/Box-Box/BBL/msanxiety/scripts/FINAL/ms_functions.R")
```


### read in emr data
```{r read_in_df}
homedir <- "~/Library/CloudStorage/Box-Box/BBL/msanxiety/"
...
#read in mimosa data
mimosa_qc <- read.csv("~/Library/CloudStorage/Box-Box/BBL/msdepression/data/melissa_martin_files/csv/mimosa_dataframe", sep = ",", header = FALSE) #n=2841
...
fascicle_proportions <- read.csv(paste0("~/Library/CloudStorage/Box-Box/BBL/msdepression//results/fascicle_volumes_all_subjects_roi_n2336.csv"), header = T, sep = ",") #n = 2336
...
lesion_volumes <- read.csv("~/Library/CloudStorage/Box-Box/BBL/msdepression/results/mimosa_binary_masks_hcp_space_20211026_n2336_volumes", sep = ",", header = TRUE) #n=2336, 958 unique, identical to fascicle unique
...
### read in brain volume size
```{r read_in_brain_volumes}
##########################################################
...
brain_volumes <- read.csv(paste0(homedir, "/results/total_fast_brain_volumes_all_subjs.csv"), sep = ",", header = TRUE) #n=2336, 
...
### merge brain vol with data frame with good mimosas
```{r merge_brain_volume_w_mimosa_df}
##########################################################
#             Merge lesion volumes with mimosa df        #
##########################################################
#merge whole data set with fascicle proportions, n = 814
df_demo_and_fascicles <- df_demo_and_fascicles %>%
  left_join(brain_volumes, by = c("EMPI", "EXAM_DATE")) 

#test to make sure age is correlated with gm and tbv
gmv_cor_age <- cor.test(df_demo_and_fascicles$PAT_AGE_AT_EXAM, df_demo_and_fascicles$gm_volume)
print(gmv_cor_age) #Yup: t = -6.9769, df = 812, p-value = 6.264e-12 (higher age, lower gmv); cor = -0.237817 

tbv_cor_age <- cor.test(df_demo_and_fascicles$PAT_AGE_AT_EXAM, df_demo_and_fascicles$total_volume)
print(tbv_cor_age) #Yup: t = -4.5874, df = 812, p-value = 5.196e-06 (higher age, lower tbv); cor = -0.158941 
```
...
#read in fiber volume values
fiber_volume_values <- read.csv("~/Library/CloudStorage/Box-Box//BBL/msdepression/templates/dti/HCP_YA1065_tractography/fiber_volume_values.csv", sep=",", header = T)
...
neurosynth_anxiety_network_fascicle_overlap <- read.csv(paste0(homedir, "/results/streamline_volume_within_anxiety_network_association_neuro_maps_FDR_01.csv"), header = T, sep = ",") %>% 
...
```
### MS med summaries by group
```{r demographics_meds}

#this will display a demographics table comparing age, race, sex between healthy and depressed groups, using the first presentation to treatment
#msmeds simple, all groups
#msmeds_table_all_groups <- make_ms_med_summary_table_ms_anxiety_simple(data_frame = df_demo_and_fascicles[df_demo_and_fascicles$oanxietydx_OR_meds_AND_Anxietydx.Meds.And.Dep.True.Healthy.Collapsed != "unclassified",])

#save 
#msmeds_table_all_groups_df <- print(msmeds_table_all_groups, as.data.frame = TRUE)
#write.csv(msmeds_table_all_groups_df, file = paste0(homedir, "/results/msmeds_table_all_groups_df.csv"), row.names = TRUE)

#msmeds simple, just MS+noA and MS+severeA
#msmeds_table_diagnosis <- make_ms_med_summary_table_ms_anxiety_simple(data_frame = df_demo_and_fascicles[((df_demo_and_fascicles$oanxietydx_OR_meds_AND_Anxietydx.Meds.And.Dep.True.Healthy.Collapsed != "unclassified") & (df_demo_and_fascicles$oanxietydx_OR_meds_AND_Anxietydx.Meds.And.Dep.True.Healthy.Collapsed != "Has.Anxietydx.OR.Antianxiety.Meds")),])

#save 
#msmeds_table_diagnosis_df <- print(msmeds_table_diagnosis, as.data.frame = TRUE)
#write.csv(msmeds_table_diagnosis_df, file = paste0(homedir, "/results/msmeds_table_diagnosis_df.csv"), row.names = TRUE)
```
...
write.csv(simple_demo_promis_diagnosis_df, file = paste0(homedir, "/results/simple_demo_promis_diagnosis_df.csv"), row.names = TRUE)
...
```{r plot_Promis_x_anxiety_mental_vs_phys}
#set mood versus physical domains
...
df_for_promis_plot_mental_vs_physical_healthy <- df_people_with_promis_data_scores_added_unique %>%
  group_by(EMPI) %>%
  mutate("Physical Functioning" = sum(across(physical_domains))/length(physical_domains)) %>%
  mutate("Emotional Functioning" = sum(across(mood_domains))/length(mood_domains)) %>%
  dplyr::select(EMPI,
                anxiety_dose,
...
desired_order <- c("Physical Functioning", "Emotional Functioning")
...
custom_colors_simple <- c("Physical Functioning" = "#4B0082",  # Purple
                          "Emotional Functioning" = "#9ACD32")  # Green
...
#save eps
postscript(file = paste0(homedir, "/results/Figures_for_msanxiety_paper/promis_with_post_op_mental_vs_physical.eps"), width = 2, height = 1.5)
  promis_with_post_op_mental_vs_physical
dev.off()
```
####Test promis*anxiety dose  stats
#####NEW JAMA ANALYSIS: Testing for Normality - all normally distributed
```{r normality_testing}
###
...
### MS noA
## MS+noA, Physical -> W = 0.87548, p-value = 0.07673 (normal)
ms_noa_physical_normality <- shapiro.test(df_for_promis_plot_mental_vs_physical_healthy_test$physical_health_score[df_for_promis_plot_mental_vs_physical_healthy_test$anxiety_dose == 0])
print(ms_noa_physical_normality)
## MS+noA, Emotional -> W = 0.92006, p-value = 0.2864 (normal)
ms_noa_emotional_normality <- shapiro.test(df_for_promis_plot_mental_vs_physical_healthy_test$mental_health_score[df_for_promis_plot_mental_vs_physical_healthy_test$anxiety_dose == 0])
print(ms_noa_emotional_normality)

### MS MILD
## MS+mildA, Physical -> W = 0.96068, p-value = 0.1297 (normal)
ms_milda_physical_normality <- shapiro.test(df_for_promis_plot_mental_vs_physical_healthy_test$physical_health_score[df_for_promis_plot_mental_vs_physical_healthy_test$anxiety_dose == 1])
print(ms_milda_physical_normality)
## MS+mildA, Emotional -> W = 0.97038, p-value = 0.299 (normal)
ms_milda_emotional_normality <- shapiro.test(df_for_promis_plot_mental_vs_physical_healthy_test$mental_health_score[df_for_promis_plot_mental_vs_physical_healthy_test$anxiety_dose == 1])
print(ms_milda_emotional_normality)

### MS SEVERE
## MS+severeA, Physical -> W = 0.96162, p-value = 0.8325 (normal)
ms_severea_physical_normality <- shapiro.test(df_for_promis_plot_mental_vs_physical_healthy_test$physical_health_score[df_for_promis_plot_mental_vs_physical_healthy_test$anxiety_dose == 2])
print(ms_severea_physical_normality)
## MS+severeA, Emotional -> W = 0.89898, p-value = 0.3248 (normal)
ms_severea_emotional_normality <- shapiro.test(df_for_promis_plot_mental_vs_physical_healthy_test$mental_health_score[df_for_promis_plot_mental_vs_physical_healthy_test$anxiety_dose == 2])
print(ms_severea_emotional_normality)

normality_stats_table <- data.frame(matrix(nrow = 6, ncol = 5))
names(normality_stats_table) <- c("Anxiety Severity", "PROMIS Functioning Domain", "W", "P", "Normality")
normality_stats_table$`Anxiety Severity` <- rep(c("MS+noA", "MS+mildA", "MS+severeA"), 2)
normality_stats_table$`PROMIS Functioning Domain` <- rep(c("Physical", "Emotional"), 3)

count = 1
for (anxiety_dose in 0:2){
  for (promis in c("physical_health_score", "mental_health_score"))
  {
    normality_test <- eval(parse(text = paste0("shapiro.test(df_for_promis_plot_mental_vs_physical_healthy_test$", promis, "[df_for_promis_plot_mental_vs_physical_healthy_test$anxiety_dose == ", anxiety_dose, "])")))
    print(normality_test)
    normality_stats_table$W[count] = round(normality_test$statistic, 3)
    normality_stats_table$P[count] = round(normality_test$p.value, 3)
    normality_stats_table$Normality = ifelse(normality_test$p.value < 0.05, "NON-NORMAL", "NORMAL")
    count = count + 1
  }
}
print(normality_stats_table)

write.csv(normality_stats_table, paste0(homedir, "results/normality_table_promis.csv"), row.names = F, sep = ",", col.names = T)
...
### confint
 # Function to compute Cohen's f² on bootstrapped samples
cohens_f2 <- function(data, indices) {
  d <- data[indices, ]  # Resample rows
  
  # Explicitly using relevant columns
  full_model <- lm(physical_health_score ~ anxiety_dose, data =d)
  
  # Extract R² values
  R2 <- summary(full_model)$r.sq
  
  # Compute Cohen's f² R2/1-R2
  f2 <- round((R2 / (1 - R2)), 3)
  

  return(f2)  # The statistic returned by boot()
}
  
 
# Perform bootstrapping
set.seed(123)  # Ensure reproducibility
boot_results <- boot(data = df_for_promis_plot_mental_vs_physical_healthy_test, statistic = cohens_f2, R = 1000)

# Compute 95% CI for Cohen's f²
ci_results_physical <- boot.ci(boot_results, type = "perc")
print(ci_results_physical)
...
cohens_f2 <- function(data, indices) {
  ...
  # Explicitly using relevant columns
  full_model <- lm(mental_health_score ~ anxiety_dose, data =d)
...
# Compute 95% CI for Cohen's f²
ci_results_mental <- boot.ci(boot_results, type = "perc")
print(ci_results_mental)
...
#looking at anxiety_dose = 0, 1 and 2  

# dose 2, "(T = 2.88, df = 6, P = 0.028, Cohen's d =  -0.69, Effect size = medium)"
physical_vs_mental_health_anxiety_severeA <- t_p_and_cohen_d(sample_1 = df_for_promis_plot_mental_vs_physical_healthy_test$physical_health_score[df_for_promis_plot_mental_vs_physical_healthy_test$anxiety_dose ==2], sample_2 = df_for_promis_plot_mental_vs_physical_healthy_test$mental_health_score[df_for_promis_plot_mental_vs_physical_healthy_test$anxiety_dose ==2], paired = "T")
print(physical_vs_mental_health_anxiety_severeA[1])

#dose 1, "(T = 2.97, df = 44, P = 0.005, Cohen's d =  -0.38, Effect size = small)"

physical_vs_mental_health_anxiety_mildA <- t_p_and_cohen_d(sample_1 = df_for_promis_plot_mental_vs_physical_healthy_test$physical_health_score[df_for_promis_plot_mental_vs_physical_healthy_test$anxiety_dose ==1], sample_2 = df_for_promis_plot_mental_vs_physical_healthy_test$mental_health_score[df_for_promis_plot_mental_vs_physical_healthy_test$anxiety_dose ==1], paired = "T")
print(physical_vs_mental_health_anxiety_mildA[1])

#dose 0, "(T = 0.68, df = 11, P = 0.513, Cohen's d =  -0.22, Effect size = small)"

physical_vs_mental_health_anxiety_noA <- t_p_and_cohen_d(sample_1 = df_for_promis_plot_mental_vs_physical_healthy_test$physical_health_score[df_for_promis_plot_mental_vs_physical_healthy_test$anxiety_dose ==0], sample_2 = df_for_promis_plot_mental_vs_physical_healthy_test$mental_health_score[df_for_promis_plot_mental_vs_physical_healthy_test$anxiety_dose ==0], paired = "T")
print(physical_vs_mental_health_anxiety_noA[1])

#looking at physical health and mental health in sick vs healthy

###physical health, sick vs healthy - "(T = 2.00, df = 8.66, P = 0.077, Cohen's d =  -1.01, Effect size = large)"

physical_health_noAvsSevereA <- t_p_and_cohen_d(sample_1 = df_for_promis_plot_mental_vs_physical_healthy_test$physical_health_score[df_for_promis_plot_mental_vs_physical_healthy_test$anxiety_dose ==0], sample_2 = df_for_promis_plot_mental_vs_physical_healthy_test$physical_health_score[df_for_promis_plot_mental_vs_physical_healthy_test$anxiety_dose ==2], paired = "F")
print(physical_health_noAvsSevereA[1])

###mental health, sick vs healthy - (T = 3.68, df = 8.13, P = 0.006, Cohen's d =  -1.87, Effect size = large)
mental_health_noAvsSevereA <- t_p_and_cohen_d(df_for_promis_plot_mental_vs_physical_healthy_test$mental_health_score[df_for_promis_plot_mental_vs_physical_healthy_test$anxiety_dose ==0], df_for_promis_plot_mental_vs_physical_healthy_test$mental_health_score[df_for_promis_plot_mental_vs_physical_healthy_test$anxiety_dose ==2], paired="F")
print(mental_health_noAvsSevereA[1])

#construct df of p values
t_p_and_cohen_values <- t(data.frame(rbind(c(physical_vs_mental_health_anxiety_noA[2],
                         physical_vs_mental_health_anxiety_mildA[2],
                         physical_vs_mental_health_anxiety_severeA[2],
                         mental_health_noAvsSevereA[2],
                         physical_health_noAvsSevereA[2]),
                         c(physical_vs_mental_health_anxiety_noA[3],
                         physical_vs_mental_health_anxiety_mildA[3],
                         physical_vs_mental_health_anxiety_severeA[3],
                         mental_health_noAvsSevereA[3],
                         physical_health_noAvsSevereA[3]),
                         c(physical_vs_mental_health_anxiety_noA[4],
                         physical_vs_mental_health_anxiety_mildA[4],
                         physical_vs_mental_health_anxiety_severeA[4],
                         mental_health_noAvsSevereA[4],
                         physical_health_noAvsSevereA[4])), 
                         row.names = c("t", "p", "cohen_d")))

#construct vector of names
t_test_names <- c("Physical vs Emotional Functioning MS+noA", 
"Physical vs Emotional Functioning MS+mildA",
"Physical vs Emotional Functioning MS+severeA",
"Emotional Functioning MS+severeA vs MS+noA",
"Physical Functioning MS+severeA vs MS+noA")

rownames(t_p_and_cohen_values) <- t_test_names

#fdr correct
t_p_and_cohen_values <- as.data.frame(t_p_and_cohen_values) %>%
  mutate(t = as.numeric(t)) %>%
  mutate(fdr = round(p.adjust(p, method = "fdr"),3)) %>%
  mutate(p = as.numeric(p)) %>%
  mutate(cohen_d = as.numeric(cohen_d)) %>%
  relocate(cohen_d, .after = fdr)

print(t_p_and_cohen_values)

#write output, will be used in supplement
write.csv(t_p_and_cohen_values, file = paste0(homedir, "/results/promis_pairwise_t_p_and_cohen_d_values.csv"), row.names = TRUE)

#save ones that meet correction
pfdr_0.05 <- t_p_and_cohen_values %>%
  filter(fdr <0.05)

print(pfdr_0.05)

#bonferroni correct
#pbonf <- round(p.adjust(p_values, method = "bonferroni"),3)
#pbonf_0.05 <- pbonf[pbonf < 0.05]
#print(pbonf_0.05)
...
depression_empis <- read.table(file=paste0(homedir, "/data/empis_of_people_in_BP_depression_paper.csv"))
...
### Lesion volumes 
#### Diagnosis - sick > healthy in vol of lesions
```{r lesion_volumes_dx}
...
#gam lesion volumes; DiagnosisMS+Anxiety 8.262e+03  2.691e+03   3.070  0.00266 **
volume_of_mimosa_lesions_gam <- gam(volume_of_mimosa_lesions ~ Diagnosis + osex + s(PAT_AGE_AT_EXAM, k = 4, fx = T) + total_volume, data = df_empi_dx_network_proportions)
summary(volume_of_mimosa_lesions_gam)

 #print summary stats; (T = 3.07, P = 0.003, Cohen's f2 =  0.354, Effect size = large)
 gam_summary_stats(volume_of_mimosa_lesions_gam)
```


#### Anxiety dose- anxiety dose correlates with disease burden (volume of lesions)
...
df_demo_and_fascicles_no_unclass_anxiety_dose <- df_demo_and_fascicles %>%
 ...
 #bilat anxiety dose + sex + age:  anxiety_dose  3.955e+03  1.228e+03   3.221  0.00139 ** 

  anxiety_dose_volume_of_mimosa_lesions_by_sex_gam <- gam(volume_of_mimosa_lesions ~ anxiety_dose + osex + s(PAT_AGE_AT_EXAM, k = 4, fx = T) + total_volume, data=df_demo_and_fascicles_no_unclass_anxiety_dose)
  summary(anxiety_dose_volume_of_mimosa_lesions_by_sex_gam)
  
   #print summary stats; (T = 3.221, P = 0.001, Cohen's f2 =  0.168, Effect size = medium)
...
```{r tbv_dx}
 df_empi_dx_network_proportions <- df_demo_and_fascicles %>%
  ...
  mutate(Diagnosis = ifelse(oanxietydx_OR_meds_AND_Anxietydx.Meds.And.Dep.True.Healthy.Collapsed=="true_healthy", "MS-Anxiety", "MS+Anxiety")) %>%
  mutate(Diagnosis = as.factor(Diagnosis)) %>%
  dplyr::select(subject, osex, Diagnosis, PAT_AGE_AT_EXAM, total_volume) %>%
  ungroup() #n=123
 
##### healthy vs sick

#gam brain volume; DiagnosisMS+Anxiety     3359      25590   0.131    0.896    
#(T = 0.131, P = 0.896, Cohen's f2 =  0.4, Effect size = large)
total_brain_volume_gam <- gam(total_volume ~ Diagnosis + osex + s(PAT_AGE_AT_EXAM, k = 4, fx = T), data = df_empi_dx_network_proportions)
summary(total_brain_volume_gam)

 #print summary stats
 gam_summary_stats(total_brain_volume_gam)
...
#### Anxiety dose
  - anxiety dose correlates with disease burden (total brain volume )
```{r tbv_anxiety_dose}
df_demo_and_fascicles_no_unclass_anxiety_dose <- df_demo_and_fascicles %>%
  filter(oanxietydx_OR_meds_AND_Anxietydx.Meds.And.Dep.True.Healthy.Collapsed != "unclassified") %>%
  filter (anxiety_dose != -1) #n=372
 ...
 #brain volume anxiety dose + sex + age: anxiety_dose     3948      11484   0.344    0.731  
#(T = 0.344, P = 0.731, Cohen's f2 =  0.311, Effect size = medium)

  anxiety_dose_total_brain_volume_by_sex_gam <- gam(total_volume ~ anxiety_dose + osex + s(PAT_AGE_AT_EXAM, k = 4, fx = T), data=df_demo_and_fascicles_no_unclass_anxiety_dose)
  summary(anxiety_dose_total_brain_volume_by_sex_gam)
  
   #print summary stats
 gam_summary_stats(anxiety_dose_total_brain_volume_by_sex_gam)
...
summary(disease_mean_UF_prop_gam)
 ...
 #print summary stats; (T = 2.022, P = 0.045, Cohen's f2 =  0.192, Effect size = medium)
...
#results: Anxiety dose related to UF_R when covarying for linear effects of age and sex 
...
df_demo_and_fascicles_no_unclass_anxiety_dose <- df_demo_and_fascicles %>%
  ...
  filter(anxiety_dose != -1) #n=372
    ...
    #bilat anxiety dose + sex + age + total volume: anxiety_dose  0.04989    0.02397   2.082   0.0381 * 
 anxietyGrouping_mean_UF_by_sex_gam_prop <- gam(mean_UF_prop ~ anxiety_dose + osex + s(PAT_AGE_AT_EXAM, k = 4, fx = F), data=df_demo_and_fascicles_no_unclass_anxiety_dose)
  ...
  #print summary stats; (T = 2.082, P = 0.038, Cohen's f2 =  0.105, Effect size = small)
...
#Fornix gam Diagnosis; DiagnosisMS+Anxiety   (T = 0.67, P = 0.504, Cohen's f2 =  0.025, Effect size = small)
anxietyDiagnosis_mean_F_by_sex_gam_prop <- gam(mean_fornix_prop ~ Diagnosis + osex + s(PAT_AGE_AT_EXAM, k = 4, fx = T) + total_volume, data = df_empi_dx_network_proportions)
...
df_demo_and_fascicles_no_unclass_anxiety_dose <- df_demo_and_fascicles %>%
  ...
  #gam, Fornix anxiety_dose  ; anxiety_dose  2.305e-02  2.404e-02   0.959   0.3383 

  anxietyGrouping_mean_F_by_sex_gam_prop <- gam(mean_fornix_prop ~ anxiety_dose + osex + s(PAT_AGE_AT_EXAM, k = 4, fx = T) + total_volume, data=df_demo_and_fascicles_no_unclass_anxiety_dose)
  summary(anxietyGrouping_mean_F_by_sex_gam_prop)
  
 #print summary stats
 gam_summary_stats(anxietyDiagnosis_mean_F_by_sex_gam_prop) #"(T = 0.67, P = 0.504, Cohen's f2 =  0.025, Effect size = small)
 gam_summary_stats(anxietyGrouping_mean_F_by_sex_gam_prop) #(T = 0.959, P = 0.338, Cohen's f2 =  -0.003, Effect size = NA
```

###### UF by depGroupVar (vol/prop, N.S)
```{r UF_by_depGroupVar}
...
table(df_dep$depDiagnosis) #MS+Dep n = 207; MS+noDep n = 99
 ...
 #mean UF, depDiagnosisMS+noDepression  T = -1.659, P = 0.098, Cohen's f2 =  0.105
dep_mean_UF_gam_prop <- gam(mean_UF_prop ~ depDiagnosis + osex + s(PAT_AGE_AT_EXAM, k=4, fx=F) + total_volume, data = df_dep)
summary(dep_mean_UF_gam_prop)

  #print summary stats: (T = -1.659, P = 0.098, Cohen's f2 =  0.105, Effect size = small)
 gam_summary_stats(dep_mean_UF_gam_prop)
 
 
```

###### JAMA ANALYSIS: Lesion & brainvol by depGroupVar (lesion vol - Y, tbv - N, N.S)
```{r UF_by_depGroupVar_lesion_tbv}
# Specificity - want to make sure the lesion and brain vol findings are nonspecific t so check to see if same findings exist in depression
### Reviewed previous pull from BP paper, finding was also consistent there -> no diff in uncinate in depressed vs non depressed
...
table(df_dep$depDiagnosis) #MS+Dep n = 207; MS+noDep n = 99
 ...
 #mean vol mimosa lesions, depDiagnosisMS+noDepression; depDiagnosisMS+noDepression -5.608e+03  1.753e+03  -3.200 0.001522 ** 

dep_mean_lesion_vol_gam_prop <- gam(volume_of_mimosa_lesions ~ depDiagnosis + osex + s(PAT_AGE_AT_EXAM, k=4, fx=F) + total_volume, data = df_dep)
summary(dep_mean_lesion_vol_gam_prop)

  #print summary stats; #(T = -3.2, P = 0.002, Cohen's f2 =  0.178, Effect size = medium)
 gam_summary_stats(dep_mean_lesion_vol_gam_prop)
...
###### JAMA ANALYSIS- UF Anxiety diagnosis + lesion vol
```{r uncinate_by_dx_lesion_vol}
### Having found that there is no association between the number of uncinate lesions and the anxiety dose, I next explored whether the *burden* of uncinate lesions differs by diagnosis/group
...
#define group
  ...
  mutate(Diagnosis = ifelse(oanxietydx_OR_meds_AND_Anxietydx.Meds.And.Dep.True.Healthy.Collapsed=="true_healthy", "MS-Anxiety", "MS+Anxiety")) %>%
  mutate(Diagnosis = as.factor(Diagnosis)) %>%
  mutate(mean_UF_vol_scaled_total_vol_lesions = mean_UF_vol/volume_of_mimosa_lesions) %>%
  ungroup() #n=123
 

#First, test if they are related. If so, they can't really be modeled together -> 

  corr_LV_to_UF <- cor.test(uncinate_by_dx_df$volume_of_mimosa_lesions,uncinate_by_dx_df$mean_UF_vol) #corr 0.69; p-value < 2.2e-16
  print(corr_LV_to_UF)


```
###### JAMA ANALYSIS-  UF Anxiety severity + lesion vol
```{r uncinateByAnxietyDose_lesion_vol}
...
#cor = 0.6282121; t = 15.531, df = 370, p-value < 2.2e-16
corr_LV_to_UF_anxiety_dose <- cor.test(df_demo_and_fascicles_no_unclass_anxiety_dose$volume_of_mimosa_lesions,df_demo_and_fascicles_no_unclass_anxiety_dose$mean_UF_vol) #
  print(corr_LV_to_UF_anxiety_dose)

```
###### JAMA ANALYSIS- UF Anxiety diagnosis + total brain vol
```{r uncinate_by_dx_tbv}
### Having found that there is no association between the number of uncinate lesions and the anxiety dose, I next explored whether the *burden* of uncinate lesions differs by diagnosis/group
...
#bilateral UF ~ Diagnosis: DiagnosisMS+Anxiety   0.10624    0.05255   2.022   0.0455  * original result

#First, test if they are related. If so, they can't really be modeled together -> 
  
 # t = -0.34681, df = 121, p-value = 0.7293; corr -0.03151255 

  corr_tbv_to_UF <- cor.test(uncinate_by_dx_df$total_volume,uncinate_by_dx_df$mean_UF_vol) #corr 0.69; 
  print(corr_tbv_to_UF)
  
  
  #can covary, no change: "(T = 2.009, P = 0.047, Cohen's f2 =  0.19, Effect size = medium)"
  disease_UF_cov_brain_vol_gam <- gam(mean_UF_prop ~ Diagnosis + osex+s(PAT_AGE_AT_EXAM, k = 4, fx = F) + total_volume, data = uncinate_by_dx_df)
  summary(disease_UF_cov_brain_vol_gam) #- N.S.
  gam_summary_stats(disease_UF_cov_brain_vol_gam)
...
cohens_f2 <- function(data, indices) {
  ...
  # Explicitly using relevant columns
  full_model <- gam(mean_UF_prop ~ Diagnosis + osex + s(PAT_AGE_AT_EXAM, k = 4) + total_volume, data = d)
  reduced_model <- gam(mean_UF_prop ~ osex + s(PAT_AGE_AT_EXAM, k = 4) + total_volume, data = d)
...
# Perform bootstrapping
set.seed(123)  # Ensure reproducibility
boot_results <- boot(data = uncinate_by_dx_df, statistic = cohens_f2, R = 1000)

# Compute 95% CI for Cohen's f²; 95%   ( 0.0810,  0.5209 ) 
...
###### JAMA ANALYSIS-  UF Anxiety dose + total brain volume
```{r uncinateByAnxietyDose_tbv}
...
#check correlations  
# cor: -0.06590961 ; t = -1.2706, df = 370, p-value = 0.2047
  corr_tbv_to_UF <- cor.test(df_demo_and_fascicles_no_unclass_anxiety_dose$total_volume,df_demo_and_fascicles_no_unclass_anxiety_dose$mean_UF_vol) #corr 0.69; 
  print(corr_tbv_to_UF)
  
  
  #can covary, no change: (T = 2.089, P = 0.037, Cohen's f2 =  0.102, Effect size = small)
  disease_UF_cov_brain_vol_dose_gam <- gam(mean_UF_prop ~ anxiety_dose + osex+s(PAT_AGE_AT_EXAM, k = 4, fx = F) + total_volume, data = df_demo_and_fascicles_no_unclass_anxiety_dose)
  summary(disease_UF_cov_brain_vol_dose_gam) #- N.S.
  gam_summary_stats(disease_UF_cov_brain_vol_dose_gam)
...
cohens_f2 <- function(data, indices) {
  ...
  # Explicitly using relevant columns
  full_model <- gam(mean_UF_prop ~ anxiety_dose + osex + s(PAT_AGE_AT_EXAM, k = 4) + total_volume, data = d)
...
# Perform bootstrapping
set.seed(123)  # Ensure reproducibility
boot_results <- boot(data = df_demo_and_fascicles_no_unclass_anxiety_dose, statistic = cohens_f2, R = 1000)

# Compute 95% CI for Cohen's f²: 95%   ( 0.051,  0.210 ) 
ci_results <- boot.ci(boot_results, type = "perc")
print(ci_results)
...
##### JAMA ANALYSIS- correction for multiple comparisons
```{r sensi_correction}
#uf dep group
#fornix by anxiety diag
#fornix by anxiety dose
#lesion vol anxiety diag
#lesion vol anxiety dose

anxiety_dx_vol_mimosa_lesions = summary(volume_of_mimosa_lesions_gam)$p.table[2,4]
anxiety_dose_vol_mimosa_lesions = summary(anxiety_dose_volume_of_mimosa_lesions_by_sex_gam)$p.table[2,4]
dep_mean_lesion_vol_gam = summary(dep_mean_lesion_vol_gam_prop)$p.table[2,4]
dep_uf_gam = summary(dep_mean_UF_gam_prop)$p.table[2,4]
anxiety_dx_fornix = summary(anxietyDiagnosis_mean_F_by_sex_gam_prop)$p.table[2,4]
anxiety_dose_fornix = summary(anxietyGrouping_mean_F_by_sex_gam_prop)$p.table[2,4]

p_vals <- array(c(anxiety_dx_vol_mimosa_lesions,
                 anxiety_dose_vol_mimosa_lesions,
                 dep_mean_lesion_vol_gam,
                 dep_uf_gam,
                 anxiety_dx_fornix,
                 anxiety_dose_fornix))

#0.005324721 0.004567230 0.004567230 0.147282059 0.496139621 0.405577736
p_val_fdr <- p.adjust(method = "fdr", p_vals)
names(p_val_fdr) = c("anxiety_dx_vol_mimosa_lesions", "anxiety_dose_vol_mimosa_lesions", "dep_mean_lesion_vol_gam", "dep_uf_gam", "anxiety_dx_fornix", "anxiety_dose_fornix")
print(p_val_fdr)
...
#fade colors at three different fade levels
#custom_colors_faded <- addFade(col = custom_colors, fade = c(0.1, 0.5, 1))

#change fill colors with progressive fades
#custom_color_fill <- c(
#  custom_colors_faded[1,1],
#  custom_colors_faded[2,2],
#  custom_colors_faded[3,3] 
#)
...
prop_injured_mean_uncinate_by_anxiety_burden_collapsed_vars_plot <- ggplot(df_disease_burden_unc_and_anx_net_collapsed_variables, aes(x = anxiety_severity, y = mean_UF_prop, color = anxiety_severity)) +
  geom_boxplot(aes(fill = anxiety_severity), alpha = 0.65, outlier.size = 0, linewidth=0.8, width = 0.5) +  # Add boxplots with some transparency
  scale_color_manual(values = custom_colors) +  # Apply custom color scale
  scale_fill_manual(values = custom_colors) +  # Apply custom fill scale
  theme(
    axis.text.x = element_text(size = 12, colour = "black"),
    axis.text.y = element_text(size = 12, colour = "black"),
...
#save pdf
pdf(file=paste0(homedir, "/results/Figures_for_msanxiety_paper/UF_anxiety_severity_box_plot_no_jitter.pdf"))
  prop_injured_mean_uncinate_by_anxiety_burden_collapsed_vars_plot
dev.off()

postscript(file = paste0(homedir, "/results/Figures_for_msanxiety_paper/UF_anxiety_severity_box_plot_no_jitter.eps"), width = 2, height = 1.5)
  prop_injured_mean_uncinate_by_anxiety_burden_collapsed_vars_plot
dev.off()
